...
int ConfigManager::parse(const char *sFileName)
{
	...
	syslog(LOG_DEBUG, "Reading configuration file: '%s'", sFileName);
	...
	{
		syslog(LOG_DEBUG, "Processing configuration line: '%s'", buffer);
		...
		else if (buffer[nIndex] == '[')
		{
			...
			syslog(LOG_DEBUG, "Setting current Section: '%s'", lastSection);
		}
		else
		{
			...
			syslog(LOG_DEBUG, "Adding configuration entry: Section '%s', Key '%s', Value '%s'", lastSection, key, value);
